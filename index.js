...
async function fetchNowPlayingWithFallback() {
  ...
  async function fetchMulti(q) {
    ...
    try {
      const searches = [
        tfetch('/search/multi?language=ko-KR&query=' + encodeURIComponent(qNorm))
      ];
      if (qNorm.includes(' ')) {
        /* 띄어쓰기 포함 입력 → 공백 제거 버전도 검색 */
        searches.push(tfetch('/search/multi?language=ko-KR&query=' + encodeURIComponent(qNoSp)));
      } else if (qNoSp.length >= 3) {
        /* 띄어쓰기 없이 입력 → 앞 2글자로 넓은 후보군 확보 */
        const prefix = qNoSp.slice(0, 2);
        searches.push(tfetch('/search/multi?language=ko-KR&query=' + encodeURIComponent(prefix)));
      }
 ...
 async function loadBox({ resetScroll = false, retryLeft = 1 } = {}) {
  ...
  async function loadTrending() {
    try {
      const data = await tfetch('/trending/all/day?language=ko-KR');
...
/* ── Pick button click ── */
    btnEl.addEventListener('click', async () => {
      ...
      try {
        ...
        const discUrl = '/discover/movie?language=ko-KR'
          + '&with_genres=' + genreId
          + '&sort_by=' + sortBy
          + '&vote_count.gte=200'
          + '&include_adult=false'
        ...
        if (!pool.length) {
          ...
          const fb = await tfetch('/discover/movie?language=ko-KR'
        ...
        const [detailRes, creditsRes, kwRes] = await Promise.allSettled([
          tfetch('/movie/' + movie.id + '?language=ko-KR'),
          tfetch('/movie/' + movie.id + '/credits?language=ko-KR'),
          tfetch('/movie/' + movie.id + '/keywords'),
        ]);
